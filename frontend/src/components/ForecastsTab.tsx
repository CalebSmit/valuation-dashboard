...
function EditableCell({
  ...
  return (
    <button
      type="button"
      className="px-1 py-0.5 text-xs font-mono text-[#4493F8] hover:text-[#00FF88] cursor-pointer bg-transparent border-none text-right"
          ...
          <table className="w-full text-xs font-mono">
            <thead>
              <tr className="row-b">
                ...
                {years.map(y => (
                  <th key={y} className="text-right py-1 px-2 clr-muted">{y}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {accounts.map(acct => (
                <tr key={acct} className={`row-b ${isTotalRow(acct) ? 'font-semibold' : ''}`}>
                  ...
                  {data[acct].map((val, i) => (
                    <td key={i} className={`text-right py-1 px-2 ${isTotalRow(acct) ? 'clr-accent' : 'clr-text'}`}>
                      {formatNum(val)}
                    </td>
                  ))}
                </tr>
...
export function ForecastsTab({
  ...
  return (
    <div className="flex flex-col gap-5">
              ...
              <tr className="row-b">
                ...
                {revenueForecasts.map(rf => (
                  <td key={rf.year} className="text-right py-1 px-2 clr-accent" title={rf.rationale}>
                    {formatNum(rf.value)}<ConfidenceDot level={rf.confidence} />
                  </td>
                ))}
              </tr>
              <tr className="row-b">
                ...
                {ebitMargins.map(em => (
                  <td key={em.year} className="text-right py-1 px-2 clr-text" title={em.rationale}>
                    {formatPct(em.value)}<ConfidenceDot level={em.confidence} />
                  </td>
                ))}
              </tr>
              <tr className="row-b">
      ...
      {/* Mechanical Assumptions (editable) */}
      <div className="card p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs clr-muted">CapEx % Rev</span>
      ...
      <div className={`px-3 py-2 text-xs font-mono rounded ${validation.balanced ? 'bg-[#3FB95020] text-[#3FB950]' : 'bg-[#F8514920] text-[#F85149]'}`}>
        {validation.balanced
          ? `Balance sheet balanced (Y1-Y5, max diff: $${validation.maxDiff.toFixed(2)})`
      ...
      {/* DCF Inputs */}
      <div className="card p-3">
        <h4 className="text-xs uppercase tracking-wider mb-2 font-mono clr-muted">DCF Inputs (Derived)</h4>
        <div className="overflow-x-auto">
          <table className="w-full text-xs font-mono">
            <thead>
              <tr className="row-b">
